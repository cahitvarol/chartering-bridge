...
# =====================================================================
# YARDIMCI FONKSİYONLAR (Güvenlik ve Format)
# =====================================================================
...
st.markdown("""
    ...
    /* Sefer Kaydı butonunu mavi yapan CSS (Streamlit >=1.31 gerektirir) */
    .st-key-save_voyage_container button {
        background-color: #1976D2 !important;
        color: white !important;
        border-color: #1976D2 !important;
    }
    .st-key-save_voyage_container button:hover {
        background-color: #1565C0 !important;
        border-color: #1565C0 !important;
        color: white !important;
    }
    .st-key-save_voyage_container button:disabled {
...
if "save_conflict_id" not in st.session_state:
...
def fetch_vessel_data():
    ...
    try:
        ...
        if data and len(data) > 0:
            ...
            st.session_state.v_dwt = safe_float(v.get("dwt", 0.0))
            st.session_state.v_dwcc = safe_float(v.get("dwcc", 0.0))
            st.session_state.v_grain = safe_float(v.get("grain_cap_-cuft-", 0.0))
            st.session_state.v_bale = safe_float(v.get("bale_cap_-cuft-", 0.0))
            st.session_state.v_gt = safe_float(v.get("gross_tonnage", 0.0))
            st.session_state.v_nt = safe_float(v.get("net_tonnage", 0.0))
            st.session_state.v_loa = safe_float(v.get("loa", 0.0))
            st.session_state.v_beam = safe_float(v.get("beam", 0.0))
...
# DÜZELTME: Artık cache'li — her rerun'da değil, en fazla 30 saniyede bir Supabase'e sorgu atar.
...
def get_saved_voyages():
    ...
    try:
        ...
        # Sadece cache miss anında (gerçek sorgu denemesinde) gösterilir.
        st.warning(f"Geçmiş sefer listesi alınamadı: {e}")
        return []

def voyage_id_exists(vid):
    """Verilen voyage_id veritabanında zaten var mı kontrol eder."""
    if not supabase or not vid:
        return False
    try:
        res = supabase.table("voyage_calculations").select("voyage_id").eq("voyage_id", vid).execute()
...
def build_voyage_payload(vid):
    ...
    all_data_payload = {
        ...
        "dataframes": {
            "bunker_base": df_to_safe_dict(st.session_state.get('bunker_df')),
            "sea_base": df_to_safe_dict(st.session_state.get('sea_df')),
            "port_base": df_to_safe_dict(st.session_state.get('port_df')),
...
def save_voyage_to_db(vid):
    """Verilen ID ile sefer kaydını Supabase'e upsert eder."""
    if not supabase:
        st.error("Supabase bağlantısı kurulamadı, kayıt yapılamıyor.")
        return False
    if not vid:
        st.error("Geçersiz Voyage ID.")
        return False
    try:
        db_payload = build_voyage_payload(vid)
        supabase.table("voyage_calculations").upsert(db_payload).execute()
        get_saved_voyages.clear()  # Geçmiş sefer listesi cache'ini temizle
...
with col_right:
    ...
    edited_df = st.data_editor(
        ...
        hide_index=True, use_container_width=True, key="bunker_editor_widget"
    )
...
with sc1:
    ...
    # DÜZELTME: Select kolonu tekrar dropdown, Speed/Cons formatlı.
    current_sea = st.data_editor(
        st.session_state.sea_base,
        column_config={
            "At Sea": st.column_config.TextColumn("At Sea", disabled=True),
            "Speed": st.column_config.NumberColumn("Speed", format="%.2f knot"),
            "Cons": st.column_config.NumberColumn("Cons", format="%.2f mts"),
            "Select": st.column_config.SelectboxColumn("Select", options=yakit_tipleri, required=True)
        },
...
with sc2:
    ...
    current_port = st.data_editor(
        ...
        column_config={
            "At Port": st.column_config.TextColumn("At Port", disabled=True),
            "Cons": st.column_config.NumberColumn("Cons", format="%.2f mts"),
            "Select": st.column_config.SelectboxColumn("Select", options=yakit_tipleri, required=True)
        },
...
if 'port_rotation_base' not in st.session_state:
    st.session_state.port_rotation_base = pd.DataFrame({
        "Port Type": ["Ballast Port"], "Port Name": [""], "Distance": [0.0], "Weather Margin (%)": [5]
    })
# DÜZELTME: Distance help metni ve Weather Margin format'ı geri eklendi.
...
with btn_col:
    if st.button("Get Distance", type="primary", use_container_width=True):
        ...
        if not filtered_df.empty:
            st.session_state.port_charges_base = pd.DataFrame({"Port Type": filtered_df["Port Type"].tolist(), "Port Name": filtered_df["Port Name"].tolist(), "PDA": [0.0]*len(filtered_df), "Liner Expenses": [0.0]*len(filtered_df)})
            st.session_state.ld_details_base = pd.DataFrame({"Port Type": filtered_df["Port Type"].tolist(), "Port Name": filtered_df["Port Name"].tolist(), "Rate": [0.0]*len(filtered_df), "Unit": ["mts/day"]*len(filtered_df), "L/D Terms": ["SSHEX"]*len(filtered_df), "Extra Days": [0.0]*len(filtered_df)})
        else:
        ...
        for key in ["charges_editor_widget", "ld_editor_widget"]:
...
with col_t2:
    ...
    if 'port_charges_base' not in st.session_state:
        st.session_state.port_charges_base = pd.DataFrame({"Port Type": [""], "Port Name": [""], "PDA": [0.0], "Liner Expenses": [0.0]})
    # DÜZELTME: Port Type/Port Name tekrar disabled (Get Distance ile otomatik dolar).
...
with col_t3:
    ...
    if 'ld_details_base' not in st.session_state:
        st.session_state.ld_details_base = pd.DataFrame({"Port Type": [""], "Port Name": [""], "Rate": [0.0], "Unit": ["mts/day"], "L/D Terms": ["SSHEX"], "Extra Days": [0.0]})
    # DÜZELTME: Port Type/Port Name disabled, Rate/Extra Days format geri eklendi.
...
if hesapla_basildi:
    ...
    spd_bal = safe_float(st.session_state.sea_df.iloc[0]["Speed"])
    spd_ldn = safe_float(st.session_state.sea_df.iloc[1]["Speed"])

    if spd_bal <= 0 or spd_ldn <= 0:
        st.error("Lütfen geminin Speed (Hız) değerlerini 0'dan büyük giriniz!")
    else:
        cons_bal = safe_float(st.session_state.sea_df.iloc[0]["Cons"])
        cons_ldn = safe_float(st.session_state.sea_df.iloc[1]["Cons"])
        cons_port_work = safe_float(st.session_state.port_df.iloc[1]["Cons"])
        ...
        price_bal = safe_float(aktif_fiyatlar.get(fuel_type_bal, 0.0))
        ...
        price_ldn = safe_float(aktif_fiyatlar.get(fuel_type_ldn, 0.0))
        ...
        price_port_work = safe_float(aktif_fiyatlar.get(fuel_type_port_work, 0.0))
        ...
        for idx, row in st.session_state.ld_details_df.iterrows():
            p_name = str(row.get("Port Name", f"Port {idx+1}")).strip()
            rate = safe_float(row.get("Rate", 0.0))
            ex_days = safe_float(row.get("Extra Days", 0.0))
            ...
            cost = (p_days * cons_port_work) * price_port_work
        ...
        total_pda = safe_float(st.session_state.port_charges_df["PDA"].fillna(0).sum())
        total_liner = safe_float(st.session_state.port_charges_df["Liner Expenses"].fillna(0).sum())

        total_opex = (total_bunker_cost + total_pda + total_liner + despatch + strait_canal + extra_insurance + cargo_survey + other_exp + commissions + freight_tax)
        ...
        st.session_state.comm_multiplier = (add_comm + broker_comm) / 100.0
        ...
        st.session_state.res_summary = {"total_days": total_days, "sea_days": total_sea_days, "port_days": total_port_days, "sea_cost": total_sea_cost, "port_cost": total_port_cost}
        ...
        st.session_state.res_opex_details = [total_bunker_cost, total_pda, freight_tax, total_liner, 0.0, despatch, strait_canal, extra_insurance, cargo_survey, other_exp, gross_freight * (add_comm/100), gross_freight * (broker_comm/100), total_opex]

        # DÜZELTME: Otomatik Supabase kaydı KALDIRILDI.
        # Kayıt artık sadece kullanıcı "💾 Sefer Kaydı" butonuna bastığında yapılır (Bölüm 4).
...
with calc_col3:
    ...
    # =====================================================================
    # YENİ: RESULT tablosunun altında mavi "Sefer Kaydı" butonu
    # =====================================================================
    st.write("")
    save_container = st.container(key="save_voyage_container")
    with save_container:
        save_clicked = st.button(
            "💾 Sefer Kaydı",
            key="save_voyage_btn",
            use_container_width=True,
            disabled=not st.session_state.get("calc_done", False),
            help="Kayıt yapmak için önce CALCULATE VOYAGE ile hesaplama yapmalısınız." if not st.session_state.get("calc_done", False) else None
        )

    if save_clicked:
        vid_to_check = st.session_state.get("voyage_id_input", "").strip()
        if not vid_to_check:
            vid_to_check = generate_new_voyage_id()

        if voyage_id_exists(vid_to_check):
            # ID zaten kayıtlı -> kullanıcıya sor
            st.session_state.save_conflict_id = vid_to_check
        else:
            # Yeni ID, direkt kaydet
            if save_voyage_to_db(vid_to_check):
                st.session_state.pending_voyage_id_update = vid_to_check
                st.session_state.save_conflict_id = None
                st.rerun()

    # ID çakışması varsa: Üzerine Yaz / Yeni Oluştur / İptal seçenekleri
    if st.session_state.get("save_conflict_id"):
        st.warning(f"⚠️ '{st.session_state.save_conflict_id}' ID'li bir sefer zaten kayıtlı. Ne yapmak istersiniz?")
        conf_col1, conf_col2, conf_col3 = st.columns(3)
        with conf_col1:
            if st.button("✏️ Üzerine Yaz", key="overwrite_btn", use_container_width=True):
                vid = st.session_state.save_conflict_id
                if save_voyage_to_db(vid):
                    st.session_state.save_conflict_id = None
                    st.rerun()
        with conf_col2:
            if st.button("🆕 Yeni Oluştur", key="new_id_btn", use_container_width=True):
                new_vid = generate_new_voyage_id()
                if save_voyage_to_db(new_vid):
                    st.session_state.pending_voyage_id_update = new_vid
                    st.session_state.save_conflict_id = None
                    st.rerun()
        with conf_col3:
            if st.button("❌ İptal", key="cancel_save_btn", use_container_width=True):
                st.session_state.save_conflict_id = None
                st.rerun()
...
if st.session_state.get("calc_done", False):
    ...
    def generate_matrix_html(matrix_type, f_base, var_base, d_base, f_step, var_step, rc):
        ...
        for r_idx, f in enumerate(f_vals):
            ...
            for c_idx, v in enumerate(v_vals):
                ...
                val = get_matrix_ndp(f, v, d_base, rc) if matrix_type == 'tonnage' else get_matrix_ndp(f, st.session_state.base_q, v, rc)
    ...
    be_point = ((rc * st.session_state.base_d) + st.session_state.base_fixed_opex - st.session_state.demurrage_val) / divisor if divisor > 0 else 0.0
    ...
    with mat_col1:
        c1, c2 = st.columns(2)
        with c1: fc1 = st.number_input("Freight Change (usd)", value=st.session_state.fc_1, step=0.1, key="f_ch1")
        with c2: tc1 = st.number_input("Tonnage Change (mts)", value=st.session_state.tc_1, step=100.0, key="t_ch1")
        ...
        st.markdown(generate_matrix_html('tonnage', st.session_state.base_f, st.session_state.base_q, st.session_state.base_d, fc1, tc1, new_rc), unsafe_allow_html=True)

    with mat_col2:
        c3, c4 = st.columns(2)
        with c3: fc2 = st.number_input("Freight Change (usd)", value=st.session_state.fc_2, step=0.1, key="f_ch2")
        with c4: dc2 = st.number_input("Duration Change (days)", value=st.session_state.dc_2, step=1.0, key="d_ch2")
        ...
        st.markdown(generate_matrix_html('duration', st.session_state.base_f, st.session_state.base_d, st.session_state.base_d, fc2, dc2, new_rc), unsafe_allow_html=True)
